...
import { MoreConflicting, ClientOptions } from 'emcees-prod-testing-5';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: MoreConflicting)`
    : `const run: (${functionSource.client}: MoreConflicting) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
